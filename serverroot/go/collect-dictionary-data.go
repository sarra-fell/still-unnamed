...
func main() {
	...
	f, err := os.Open(parentDir + "/assets/some-words.txt")
	check(err)

	scanner := bufio.NewScanner(f)
	for scanner.Scan() {
		chunks := strings.Split(scanner.Text(), "+")
		relevantWords = append(relevantWords, chunks[0])
	}
	check(scanner.Err())
	...
	f, err = os.Open(parentDir + "/assets/JMdict_e")
	check(err)
	...
	f, err = os.Open(parentDir + "/assets/accents.txt")
	check(err)
	...
	f, err = os.Create(parentDir + "/assets/collected_dictionary_data.txt")
	check(err)
	defer f.Close()
